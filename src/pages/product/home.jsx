...
import { reqProduct,reqProductSelect ,reqUpdateStatus} from '../../api';
          ...
          { 
            title: '状态',
             ...
             render:(status,product)=>{
               ...
               return <>
               <Button type='primary' onClick={()=>this.UpdateChange(product._id,status)}>{status === 1 ? '下架':'上架'}</Button>
            <span>{status === 1 ? '在售':'已售'}</span>
...
UpdateChange =async (_id,status)=>{
      const res = await reqUpdateStatus(_id,status === 1 ? 2 : 1)
      if(res.status === 0) {
        this.getProduct(this.pageNumber)
      }
}

  getProduct =async (pageNumber,CON_NUMBER)=>{
    ...
    if(searchName) {
      ...
      result = await reqProductSelect({searchType,pageSize:CON_NUMBER,pageNumber,searchName})
      console.log(12313)
    }else {
      result = await reqProduct(pageNumber,CON_NUMBER)
    }
    this.setState({loading:false})
    if(result.status == 0) {
      const {list,total}=result.date
      this.setState({
        product:list,
        total
      })
...
componentDidMount() {
  ...
  render() {
    const {loading,pageSize,total,product,searchName,searchType} =this.state
    const title = (
      <>
        ...
        <Input  placeholder='关键字' style={{width:'200px'}} onChange={e=>this.setState({searchName:e.target.value})}/>
